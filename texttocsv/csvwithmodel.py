...
for filename in os.listdir(directory):
    if filename.endswith('.txt'):
        ...
        # Determine the model number based on the file name pattern
...
# Write the extracted data and model numbers to a CSV file
with open(output_csv, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    # Write the header row with additional "model" column
    header_row = ['model'] + list(aggregated_data.keys())
    writer.writerow(header_row)

    # Transpose the data and write it to the CSV
    num_samples = len(aggregated_data[list(aggregated_data.keys())[0]])
    for i in range(num_samples):
        row = [model_numbers[i]] + [aggregated_data[metric][i] for metric in aggregated_data.keys()]
        writer.writerow(row)
